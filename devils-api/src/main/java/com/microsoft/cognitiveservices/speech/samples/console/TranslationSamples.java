...
public class TranslationSamples {
    ...
    public static void translationWithMicrophoneAsync() throws InterruptedException, ExecutionException, IOException
    {
        ...
        SpeechTranslationConfig config = SpeechTranslationConfig.fromSubscription("47b288291a16497f9fc3070c74d34b39", "centralus");
    ...
    private static final String FILENAME = "speech-to-text.txt";

    public static void translationWithFileAsync() throws InterruptedException, ExecutionException, IOException
    {
...
//        System.out.println("=====>>>>> Here it came ======>");
        AudioConfig audioInput = AudioConfig.fromWavFileInput("try3.wav");
//        System.out.println("=====>>>>> Here it came Fiel read done.======>");
            ...
            recognizer.recognized.addEventListener((s, e) -> {
				...
				try {
					writer = new BufferedWriter(
					        new FileWriter("speech-to-text.txt", true)  //Set true for append mode
					    );
				} catch (IOException e1) {
                ...
                if (e.getResult().getReason() == ResultReason.TranslatedSpeech) {
                    System.out.println("RECOGNIZED in '" + fromLanguage + "': Text=" + e.getResult().getText());
						try {
							writer.newLine();
							writer.write(e.getResult().getText());
						} catch (IOException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
						}
